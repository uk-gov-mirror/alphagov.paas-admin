...
describe('applications test suite', () => {
  nock('https://example.com/api').persist()
    .get(`/v2/organizations/${data.organizationGuid}/user_roles`).times(1).reply(200, data.userRolesForOrg)
    .get(`/v2/apps/${data.appGuid}`).times(1).reply(200, data.app)
    .get(`/v2/apps/${data.appGuid}/summary`).times(1).reply(200, data.appSummary)
    .get(`/v2/apps/${data.dockerAppGuid}`).times(1).reply(200, data.dockerApp)
    .get(`/v2/apps/${data.dockerAppGuid}/summary`).times(1).reply(200, data.dockerAppSummary)
    .get(`/v2/apps/${data.cflinuxfs2AppGuid}`).times(1).reply(200, data.appUsingCflinuxfs2)
    .get(`/v2/apps/${data.cflinuxfs2AppGuid}/summary`).times(1).reply(200, data.appSummaryUsingCflinuxfs2)
    ...
    .get(`/v2/organizations/${data.organizationGuid}`).times(1).reply(200, data.organization)
